 ...
 * the buffer pool, and performs a sequential scan through all of the pages. If
 * the growth in JVM usage is greater than 2 MB due to the scan, the test fails.
 * Otherwise, the page eviction policy seems to have worked.
 */
public class EvictionTest extends SimpleDbTestBase {
    ...
    public void testHeapFileScanWithManyPages()
        ...
        if (memDiff > MEMORY_LIMIT_IN_MB) {
            Assert.fail("Did not evict enough pages.  Scan took " + memDiff
                    + " MB of RAM, when limit was " + MEMORY_LIMIT_IN_MB);
        }
    }

    public static void insertRow(HeapFile f, Transaction t)
