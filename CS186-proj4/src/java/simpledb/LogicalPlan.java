...
public class LogicalPlan {
     ...
     * 
     * @param query
     *            the text of the query associated with this plan
     */
    ...
    /**
     * Given a table alias, return id of the table object (this id can be
     * supplied to {@link Catalog#getDbFile(int)}). Aliases are added as base
     * tables are added via {@link #addScan}.
     * 
     * @param alias
     *            the table alias to return a table id for
     * @return the id of the table corresponding to alias, or null if the alias
     *         is unknown
     */
     ...
     * 
     * @param field
     *            The name of the over which the filter applies; this can be a
     *            fully qualified field (tablename.field or alias.field), or can
     *            be a unique field name without a tablename qualifier. If it is
     *            an ambiguous name, it will throw a ParsingException
     * @param p
     *            The predicate for the filter
     * @param constantValue
     *            the constant to compare the predicate against; if field is an
     *            integer field, this should be a String representing an integer
     * @throws ParsingException
     *             if field is not in one of the tables added via
     *             {@link #addScan} or if field is ambiguous (e.g., two tables
     *             contain a field named field.)
     */
    public void addFilter(String field, Predicate.Op p, String constantValue)
            throws ParsingException {

        field = disambiguateName(field);
        String table = field.split("[.]")[0];

        LogicalFilterNode lf = new LogicalFilterNode(table, field.split("[.]")[1], p,
                constantValue);
     ...
     * 
     * @param joinField1
     *            The name of the first join field; this can be a fully
     *            qualified name (e.g., tableName.field or alias.field) or may
     *            be an unqualified unique field name. If the name is ambiguous
     *            or unknown, a ParsingException will be thrown.
     * @param joinField2
     *            The name of the second join field
     * @param pred
     *            The join predicate
     * @throws ParsingException
     *             if either of the fields is ambiguous, or is not in one of the
     *             tables added via {@link #addScan}
     */
    ...
    public void addJoin(String joinField1, String joinField2, Predicate.Op pred)
        ...
        LogicalJoinNode lj = new LogicalJoinNode(table1Alias, table2Alias, pureField1,
                pureField2, pred);
     ...
     *            or unknown, a ParsingException will be thrown.
     * @param joinField2
     *            the subquery to join with -- the join field of the subquery is
     *            the first field in the result set of the query
     * @param pred
     *            The join predicate.
     * @throws ParsingException
     *             if either of the fields is ambiguous, or is not in one of the
    ...
    public void addJoin(String joinField1, DbIterator joinField2, Predicate.Op pred)
        ...
        LogicalSubplanJoinNode lj = new LogicalSubplanJoinNode(table1, pureField,
                joinField2, pred);
    ...
    /**
     * Add a scan to the plan. One scan node needs to be added for each alias of
     * a table accessed by the plan.
     * 
     * @param table
     *            the id of the table accessed by the plan (can be resolved to a
     *            DbFile using {@link Catalog#getDbFile}
     * @param name
     *            the alias of the table in the plan
     */
     ...
     * query. Fields are output by the query such that the rightmost field is
     * the first added via addProjectField.
     * 
     * @param fname
     *            the field to add to the output
     * @param aggOp
     *            the aggregate operation over the field.
     * @throws ParsingException
     */
     ...
     * SimpleDb only supports a single aggregate expression and GROUP BY field.
     * 
     * @param op
     ...
     * SimpleDb only supports a single ORDER BY field.
     * 
     * @param field
     *            the field to order by
     * @param asc
     *            true if should be ordered in ascending order, false for
     *            descending order
     * @throws ParsingException
     */
    ...
    String disambiguateName(String name) throws ParsingException {
        ...
        if (fields.length > 2)
            throw new ParsingException(
                    "Field " + name + " is not a valid field reference.");
        if (fields.length == 2)
            name = fields[1];
        if (name.equals("*"))
            return name;
        ...
        if (tableName != null)
            ...
            throw new ParsingException(
                    "Field " + name + " does not appear in any tables.");
     ...
     * 
     * @throws ParsingException
     *             if s is not a valid operator name
     */
     ...
     * 
     * @param t
     *            The transaction that the returned DbIterator will run as a
     *            part of
     * @param baseTableStats
     *            a HashMap providing a {@link TableStats} object for each table
     *            used in the LogicalPlan. This should have one entry for each
     *            table referenced by the plan, not one entry for each table
     *            alias (so a table t aliases as t1 and t2 would have just one
     *            entry with key 't' in this HashMap).
     * @param explain
     *            flag indicating whether output visualizing the physical query
     *            plan should be given.
     * @throws ParsingException
     *             if the logical plan is not valid
     * @return A DbIterator representing this plan.
     */
    public DbIterator physicalPlan(TransactionId t,
            Map<String, TableStats> baseTableStats, boolean explain)
            throws ParsingException {
        Iterator<LogicalScanNode> tableIt = tables.iterator();
        ...
        while (tableIt.hasNext()) {
            ...
            SeqScan ss = null;
            try {
                ss = new SeqScan(t, Database.getCatalog().getDbFile(table.t).getId(),
                        table.alias);
            } catch (NoSuchElementException e) {
        ...
        while (filterIt.hasNext()) {
            ...
            if (subplan == null) {
                throw new ParsingException(
                        "Unknown table in WHERE clause " + lf.tableAlias);
            }
            ...
            try {// td.fieldNameToIndex(disambiguateName(lf.fieldPureName))
                ...
                throw new ParsingException(
                        "Unknown field in filter expression " + lf.fieldQuantifiedName);
            }
            ...
            try {
                p = new Predicate(
                        subplan.getTupleDesc().fieldNameToIndex(lf.fieldQuantifiedName),
                        lf.p, f);
            } catch (NoSuchElementException e) {
            ...
            TableStats s = statsMap.get(
                    Database.getCatalog().getTableName(this.getTableId(lf.tableAlias)));

            double sel = s.estimateSelectivity(
                    subplan.getTupleDesc().fieldNameToIndex(lf.fieldQuantifiedName), lf.p,
                    f);
            filterSelectivities.put(lf.tableAlias,
                    filterSelectivities.get(lf.tableAlias) * sel);
        ...
        while (joinIt.hasNext()) {
            ...
            if (!isSubqueryJoin) {
                ...
                equivMap.put(t2name, t1name); // keep track of the fact that
                                              // this new node contains both
                                              // tables
        ...
        DbIterator node = (DbIterator) (subplanMap.entrySet().iterator().next()
                .getValue());
        ...
        for (int i = 0; i < selectList.size(); i++) {
            ...
            if (si.aggOp != null) {
                ...
                try {
                    ...
                    throw new ParsingException(
                            "Unknown field " + si.fname + " in SELECT list");
                }
                ...
                if (groupByField == null) {
                    throw new ParsingException(
                            "Field " + si.fname + " does not appear in GROUP BY list");
                }
                outFields.add(0);
                TupleDesc td = node.getTupleDesc();
                int id;
        ...
        if (hasAgg) {
            ...
            try {
                aggNode = new Aggregate(node, td.fieldNameToIndex(aggField),
                        groupByField == null ? Aggregator.NO_GROUPING
                                : td.fieldNameToIndex(groupByField),
                        getAggOp(aggOp));
            } catch (NoSuchElementException e) {
        ...
        if (hasOrderBy) {
            node = new OrderBy(node.getTupleDesc().fieldNameToIndex(oByField), oByAsc,
                    node);
        }
    ...
    public TupleDesc getTupleDesc(String alia) {
        ...
        throw new IllegalArgumentException("alia:" + alia + "\n names:" + names);
    }
}