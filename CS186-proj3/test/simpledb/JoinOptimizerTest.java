...
public class JoinOptimizerTest extends SimpleDbTestBase {
     ...
     * @param columns
     *            Each entry in tuples[] must have "columns ==
     *            tuples.get(i).size()"
     * @param colPrefix
     ...
     */
    public static HeapFile createDuplicateHeapFile(ArrayList<ArrayList<Integer>> tuples,
            int columns, String colPrefix) throws IOException {
     ...
     * estimateJoinCost.
     * 
     * 我修改了构造LogicalJoinNode的参数，这里写的是tableName1和tableName2， 但应该是t1和t2
     */
    @Test
    public void estimateJoinCostTest() throws ParsingException {
        ...
        jo = new JoinOptimizer(
                p.generateLogicalPlan(tid,
                        "SELECT * FROM " + tableName1 + " t1, " + tableName2
                                + " t2 WHERE t1.c1 = t2.c2;"),
                new Vector<LogicalJoinNode>());
        // 1 join 2
        LogicalJoinNode equalsJoinNode = new LogicalJoinNode("t1", "t2",
        ...
        jo = new JoinOptimizer(
                p.generateLogicalPlan(tid,
                        "SELECT * FROM " + tableName1 + " t1, " + tableName2
                                + " t2 WHERE t1.c1 = t2.c2;"),
                new Vector<LogicalJoinNode>());
        equalsJoinNode = new LogicalJoinNode("t2", "t1", Integer.toString(2),
                Integer.toString(1), Predicate.Op.EQUALS);
        checkJoinEstimateCosts(jo, equalsJoinNode);
        // 1 join 1
        jo = new JoinOptimizer(
                p.generateLogicalPlan(tid,
                        "SELECT * FROM " + tableName1 + " t1, " + tableName1
                                + " t2 WHERE t1.c3 = t2.c4;"),
                new Vector<LogicalJoinNode>());
        equalsJoinNode = new LogicalJoinNode("t1", "t1", Integer.toString(3),
        ...
        jo = new JoinOptimizer(
                ...
                new Vector<LogicalJoinNode>());
        equalsJoinNode = new LogicalJoinNode("t2", "t2", Integer.toString(8),
    ...
    private void checkJoinEstimateCosts(JoinOptimizer jo,
        ...
        double stats[] = getRandomJoinCosts(jo, equalsJoinNode, card1s, card2s, cost1s,
                cost2s);
    ...
    public void estimateJoinCardinality() throws ParsingException {
        ...
        */
        ...
        cardinality = j.estimateJoinCardinality(
                new LogicalJoinNode("t1", "t2", "c" + Integer.toString(3),
                        "c" + Integer.toString(4), Predicate.Op.EQUALS),
                stats1.estimateTableCardinality(0.8),
                stats2.estimateTableCardinality(0.2), true, false,
        ...
        cardinality = j.estimateJoinCardinality(
                new LogicalJoinNode("t1", "t2", "c" + Integer.toString(3),
                        "c" + Integer.toString(4), Predicate.Op.EQUALS),
                stats1.estimateTableCardinality(0.8),
                stats2.estimateTableCardinality(0.2), false, true,
                TableStats.getStatsMap());
    ...
    public void orderJoinsTest() throws ParsingException, IOException, DbException,
        ...
        HeapFile emp = SystemTestUtil.createRandomHeapFile(6, 100000, null, empTuples,
                "c");
        Database.getCatalog().addTable(emp, "emp");

        ArrayList<ArrayList<Integer>> deptTuples = new ArrayList<ArrayList<Integer>>();
        HeapFile dept = SystemTestUtil.createRandomHeapFile(3, 1000, null, deptTuples,
                "c");
        Database.getCatalog().addTable(dept, "dept");

        ArrayList<ArrayList<Integer>> hobbyTuples = new ArrayList<ArrayList<Integer>>();
        HeapFile hobby = SystemTestUtil.createRandomHeapFile(6, 1000, null, hobbyTuples,
                "c");
        ...
        stats.put("dept",
                new TableStats(Database.getCatalog().getTableId("dept"), IO_COST));
        stats.put("hobby",
                new TableStats(Database.getCatalog().getTableId("hobby"), IO_COST));
        stats.put("hobbies",
        ...
        // here because the data is random), you should use the numbers that you are
        // given. Re-computing them at runtime is generally too expensive for complex
        // queries.
        ...
        // will be in the same order as they were written in the query.
        // They just have to be in an order that uses the same operators and
        // semantically means the same thing.
        nodes.add(
                new LogicalJoinNode("hobbies", "hobby", "c1", "c0", Predicate.Op.EQUALS));
        ...
        // well; we're just doing a heuristics-based optimizer, so, only ignore the
        // really bad case where "hobbies" is the outermost node in the left-deep tree.
        Assert.assertFalse(result.get(0).t1Alias == "hobbies");

        // Also check for some of the other silly cases, like forcing a cross
        // join by "hobbies" only being at the two extremes, or "hobbies" being the
        // outermost table.
        Assert.assertFalse(
                result.get(2).t2Alias == "hobbies" && (result.get(0).t1Alias == "hobbies"
                        || result.get(0).t2Alias == "hobbies"));
    }
    ...
    public void nonequalityOrderJoinsTest() throws IOException, DbException,
        ...
        j = new JoinOptimizer(
                p.generateLogicalPlan(tid,
                        "SELECT COUNT(a.c0) FROM a, b, c, d WHERE a.c1 < b.c1 AND b.c0 = c.c0 AND c.c1 = d.c1;"),
                nodes);
        ...
        System.out.println(result.get(result.size() - 1).t1Alias);
        System.out.println(result.get(result.size() - 1).t2Alias);
