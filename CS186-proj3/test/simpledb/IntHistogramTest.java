...
public class IntHistogramTest {
     ...
     * (or, at least, reasonably small space; O(log(n)) might still work if your
     * constants are good).
     */
    @Test
    ...
    public void opGreaterThanOrEqualsTest() {
        ...
        // Be conservative in case of alternate implementations
        Assert.assertTrue(
                h.estimateSelectivity(Op.GREATER_THAN_OR_EQ, -1) > 0.999);
        Assert.assertTrue(
                h.estimateSelectivity(Op.GREATER_THAN_OR_EQ, 2) > 0.6);
        Assert.assertTrue(
                h.estimateSelectivity(Op.GREATER_THAN_OR_EQ, 3) > 0.45);
        Assert.assertTrue(
                h.estimateSelectivity(Op.GREATER_THAN_OR_EQ, 4) < 0.5);
        Assert.assertTrue(
                h.estimateSelectivity(Op.GREATER_THAN_OR_EQ, 12) < 0.001);
    }
    ...
    public void opLessThanOrEqualsTest() {
        ...
        // Be conservative in case of alternate implementations
        Assert.assertTrue(
                h.estimateSelectivity(Op.LESS_THAN_OR_EQ, -1) < 0.001);
        ...
        Assert.assertTrue(
                h.estimateSelectivity(Op.LESS_THAN_OR_EQ, 12) > 0.999);
    }
