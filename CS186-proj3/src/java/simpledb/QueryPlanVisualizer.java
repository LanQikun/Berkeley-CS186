...
public class QueryPlanVisualizer {
    ...
    static final String HASH_JOIN = "join(hash)";
    ...
    private static class SubTreeDescriptor {
        ...
        SubTreeDescriptor(SubTreeDescriptor leftChild, SubTreeDescriptor rightChild) {
    ...
    private SubTreeDescriptor buildTree(int queryPlanDepth, int currentDepth,
        ...
        if (queryPlan instanceof SeqScan) {
            ...
            if (plan instanceof Join) {
                ...
                String field2 = td.getFieldName(
                        jp.getField2() + children[0].getTupleDesc().numFields());
                ...
                SubTreeDescriptor left = this.buildTree(queryPlanDepth,
                        currentDepth + adjustDepth + 3, children[0], currentStartPosition,
                        upBarShift);
                ...
                thisNode.width = Math.max(left.width + right.width + SPACE.length(),
                ...
                String field2 = td.getFieldName(
                        jp.getField2() + children[0].getTupleDesc().numFields());
                thisNode.text = String.format("%1$s(%2$s),card:%3$d", HASH_JOIN,
                        field1 + jp.getOperator() + field2, j.getEstimatedCardinality());
                int upBarShift = parentUpperBarStartShift;
                if (HASH_JOIN.length() / 2 > parentUpperBarStartShift)
                    upBarShift = HASH_JOIN.length() / 2;
                SubTreeDescriptor left = this.buildTree(queryPlanDepth,
                        currentDepth + 3 + adjustDepth, children[0], currentStartPosition,
                        upBarShift);
                SubTreeDescriptor right = this.buildTree(queryPlanDepth,
                ...
                if (gfield == Aggregator.NO_GROUPING) {
                    thisNode.text = String.format("%1$s(%2$s),card:%3$d", a.aggregateOp(),
                ...
                SubTreeDescriptor child = this.buildTree(queryPlanDepth,
                        currentDepth + 2 + adjustDepth, children[0], currentStartPosition,
                        upBarShift);
                thisNode.upBarPosition = child.upBarPosition;
                thisNode.textStartPosition = thisNode.upBarPosition
                        - alignTxt.length() / 2;
                thisNode.width = Math.max(child.width, thisNode.textStartPosition
                        + thisNode.text.length() - currentStartPosition);
                ...
                thisNode.text = String.format("%1$s(%2$s),card:%3$d",
                        SELECT, children[0].getTupleDesc().getFieldName(p.getField())
                ...
                thisNode.text = String.format("%1$s(%2$s),card:%3$d", ORDERBY,
                ...
                thisNode.text = String.format("%1$s(%2$s),card:%3$d", PROJECT, fields,
                ...
                try {
                    ...
                    card = (Integer) plan.getClass().getMethod("getEstimatedCardinality")
                            .invoke(plan);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                ...
                int fieldIdx = 0;
                try {
                    newName = (String) plan.getClass()
                            .getMethod("newName", (Class<?>[]) null).invoke(plan);
                    fieldIdx = (Integer) plan.getClass()
                            .getMethod("renamedField", (Class<?>[]) null).invoke(plan);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                String oldName = plan.getChildren()[0].getTupleDesc()
                        .getFieldName(fieldIdx);
                thisNode.text = String.format("%1$s,%2$s->%3$s,card:%4$d", RENAME,
                        oldName, newName, plan.getEstimatedCardinality());
                int upBarShift = parentUpperBarStartShift;
    ...
    private void printTree(SubTreeDescriptor root, char[] buffer, int width) {
        ...
        if (root.leftChild != null && root.rightChild == null) {
            ...
            Arrays.fill(buffer, (textHeight + 1) * width + root.leftChild.upBarPosition,
                    (textHeight + 1) * width + root.rightChild.upBarPosition + 1, '_');
