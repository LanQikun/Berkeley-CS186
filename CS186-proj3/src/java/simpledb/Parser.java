...
public class Parser {
    ...
    public static Predicate.Op getOp(String s)
    ...
    void processExpression(TransactionId tid, ZExpression wx, LogicalPlan lp)
        ...
        if (wx.getOperator().equals("AND")) {
            ...
            if (op1const && op2const) {
                isJoin = ((ZConstant) ops.elementAt(0))
                        .getType() == ZConstant.COLUMNNAME
                        && ((ZConstant) ops.elementAt(1))
                                .getType() == ZConstant.COLUMNNAME;
            ...
            if (isJoin) { // join node
                ...
                if (!op2const) { // right op is a nested query
                    try {
                        ...
                        throw new simpledb.ParsingException(
                                "Invalid subquery " + ops.elementAt(1));
    ...
    public LogicalPlan parseQueryLogicalPlan(TransactionId tid, ZQuery q)
        ...
        for (int i = 0; i < from.size(); i++) {
            ...
            try {
                ...
                e.printStackTrace();
                throw new simpledb.ParsingException(
                        "Table " + fromIt.getTable() + " is not in catalog");
            }
        }
        ...
        for (int i = 0; i < selectList.size(); i++) {
            ...
            if (si.getAggregate() == null && (si.isExpression()
                    && !(si.getExpression() instanceof ZConstant))) {
                throw new simpledb.ParsingException(
            ...
            if (si.getAggregate() != null) {
                ...
                if (groupByField != null && !(groupByField
                        .equals(si.getTable() + "." + si.getColumn())
                        || groupByField.equals(si.getColumn()))) {
                    throw new simpledb.ParsingException(
                            "Non-aggregate field " + si.getColumn()
    ...
    // 创建physicalPlan，
    // 调用physicalPlan的updateOperatorCardinality方法，并打印查询计划树
    public Query handleQueryStatement(ZQuery s, TransactionId tId)
        ...
        DbIterator physicalPlan = lp.physicalPlan(tId, TableStats.getStatsMap(),
                explain);
    ...
    public Query handleDeleteStatement(ZDelete s, TransactionId tid)
        ...
        DbIterator op = new Delete(tid,
                lp.physicalPlan(tid, TableStats.getStatsMap(), false));
    ...
    public void handleTransactStatement(ZTransactStmt s)
        ...
        if (s.getStmtType().equals("COMMIT")) {
            ...
            inUserTrans = false;
            System.out.println(
                    "Transaction " + curtrans.getId().getId() + " committed.");
            ...
            inUserTrans = false;
            System.out.println(
                    "Transaction " + curtrans.getId().getId() + " aborted.");
            ...
            while (!quit && (line = reader.readLine("SimpleDB> ")) != null) {
                ...
                while (line.indexOf(';') >= 0) {
                    ...
                    processNextStatement(
                            new ByteArrayInputStream(statementBytes));
                    long time = System.currentTimeMillis() - startTime;
                    System.out.printf("----------------\n%.2f seconds\n\n",
                            ((double) time / 1000.0));
