        ...
        if (args[0].equals("convert")) {
            try {
                if (args.length < 3 || args.length > 5) {
                    System.err.println(
                            "Unexpected number of arguments to convert ");
                    return;
                }
                File sourceTxtFile = new File(args[1]);
                File targetDatFile = new File(
                        args[1].replaceAll(".txt", ".dat"));
                ...
                if (args.length == 3)
                    ...
                    if (typeStringAr.length != numOfAttributes) {
                        System.err.println(
                                "The number of types does not agree with the number of columns");
                        return;
                    }
                    int index = 0;
                ...
                HeapFileEncoder.convert(sourceTxtFile, targetDatFile,
                        BufferPool.PAGE_SIZE, numOfAttributes, ts,
                        fieldSeparator);
