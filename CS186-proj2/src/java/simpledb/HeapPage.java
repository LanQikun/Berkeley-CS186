...
public class HeapPage implements Page {
    ...
    private TransactionId dirtyTransactionIdId;
    ...
    public byte[] getPageData() {
        ...
        int zerolen = BufferPool.PAGE_SIZE
                - (header.length + td.getSize() * tuples.length); // -
                                                                  // numSlots
                                                                  // *
                                                                  // td.getSize();
    ...
    public void deleteTuple(Tuple t) throws DbException {
        ...
        HeapPageId heapPageId = (HeapPageId) rid.getPageId();
        if (!heapPageId.equals(this.pid)) {
            throw new DbException("tuple " + t + "is not on this page");
        }

        int num = rid.tupleno();
        // if (num < this.numSlots && this.isSlotUsed(num)
        // && tuples[num].equals(t)) {
        if (!isSlotUsed(num)) {
        ...
        return;
        // } else {
        // throw new DbException("tuple " + t + "is already empty");
        // }
    }
    ...
    private void markSlotUsed(int i, boolean value) {
        ...
        int pos = i % 8;
        if (value) {
            header[index] = (byte) (header[index] | (1 << pos));
        } else {
            header[index] = (byte) (header[index] & ~(1 << pos));
        }
    }
    ...
    private class ResultIterator implements Iterator<Tuple> {
        private int cursor;
        private int leftCount;

        public ResultIterator() {
            this.cursor = 0;
            this.leftCount = numSlots - HeapPage.this.getNumEmptySlots();
        }
        ...
        public Tuple next() {
            ...
            while (!HeapPage.this.isSlotUsed(cursor)) {
                cursor++;
            }
            leftCount--;
